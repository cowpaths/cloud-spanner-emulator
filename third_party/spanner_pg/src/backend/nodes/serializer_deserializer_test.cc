...
MATCHER(CanSerializeAndDeserialize, "") {
  ...
  // via nodeToString()
  char* parse_tree_str = nodeToString(node);

  if (parse_tree_str == nullptr) {
    *result_listener << "Couldn't serialize node";
    return false;
  }
  // It would be "a subclass of Node*", but C doesn't support subclasses, so we
  // have to cast a void*.)
  Node* deserialized_node =
      reinterpret_cast<Node*>(stringToNode(parse_tree_str));
  ...
  char* reparse_tree_str = nodeToString(node);
  bool nodes_are_equal = equal(node, deserialized_node);
